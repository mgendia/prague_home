...
class Webscraper:
    ...
    def extract_units_details(self, url_list: list):
        ...
        for url in tqdm(url_list):
            ...
            poi= req.get('poi') or []
            nearby= {item.get('name'): (item.get('lat'), item.get('lon')) for item in poi}
            ...
            if ('family house' in ad_title) or ('villa' in ad_title) or \
                ('villas' in ad_title) or ('house' in ad_title):
                unit_type= 'house'
                num_bedrooms= req.get('recommendations_data').get('room_count_cb')
            elif ('apartment' in url) or ('flat' in url):
                unit_type= 'apartment'
                result= re.findall(r'(?<=en/detail/lease/flat/)([a-z0-9\-+]+)(?=/)', url)
                num_bedrooms= result[0] if result else None
            else:
                unit_type= 'other'
                num_bedrooms= None
            ...
            floor_num= [re.findall(r'\w+', item['value']) for item in req['items']\
                                if item['name']== 'Floor']
            energy_class= [re.findall(r'(?<=^Class )([a-zA-Z]{1})', item['value']) for item in req['items']\
